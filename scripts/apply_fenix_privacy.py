...
def patch_login_data_gate(target: Path) -> None:
    ...
    s = replace_once(
        ...
        "    private val lockedLoginFactory: (domain: String) -> List<Login> = { emptyList() },\n"
    ...
    s = replace_once(
        ...
        "            return CompletableDeferred(lockedLoginFactory(domain))\n"
        "        }\n"
        "        return scope.async {",
    ...
    s = replace_once(
        ...
        "                lockedLoginFactory = { domain ->\n"
        "                    val state = context.components.core.store.state\n"
        "                    PrivatePasswordAccess.lockedLogins(\n"
        "                        domain,\n"
        "                        state.tabs.map { it.content.url } +\n"
        "                            state.customTabs.map { it.content.url },\n"
        "                    )\n"
        "                },\n"
...
def patch_origin_bound_login_picker(target: Path) -> None:
    ...
    s = replace_once(
        ...
        "    val onManageLogins: () -> Unit\n"
        "        get() = {}\n\n"
        "    /** Whether [login] is the transient authentication action. */\n"
        "    val isLockedLogin: (login: Login) -> Boolean\n"
        "        get() = { false }\n\n"
        "    /** Recovers the already-known eTLD+1 carried by the transient action. */\n"
        "    val lockedLoginLookupDomain: (login: Login) -> String?\n"
        "        get() = { null }\n\n"
        "    /** Neutral text rendered instead of a saved origin or username. */\n"
        "    val lockedLoginLabel: String\n"
        "        get() = \"\"\n\n"
        "    /** Exact current top-level web origin, or null to fail closed. */\n"
        "    val currentLoginOrigin: () -> String?\n"
        "        get() = { null }\n\n"
        ...
        "    ) -> Unit\n"
        "        get() = { _, _, _, onResult -> onResult(emptyList()) }\n",
    ...
    new = (
        ...
        "    private val lockedLoginLookupDomain: (Login) -> String? = { null },\n"
        "    private val lockedLoginLabel: String = \"\",\n"
        "    private val currentLoginOrigin: () -> String? = { null },\n"
        "    private val requestUnlock: () -> Unit = {},\n"
        "    private val fetchUnlockedLogins: (String, String, Boolean, (List<Login>) -> Unit) -> Unit =\n"
        "        { _, _, _, onResult -> onResult(emptyList()) },\n"
        ") : SelectablePromptView.Listener<Login> {\n"
        "    private var lockedRequestLogin: Login? = null\n"
        "    private var pendingLookupDomain: String? = null\n"
        ...
        "        val gate = lockedRequestLogin?.takeIf(isLockedLogin) ?: option\n"
        "        val origin = currentLoginOrigin()?.takeIf { it == gate.origin } ?: return\n"
        "        val lookupDomain = lockedLoginLookupDomain(gate) ?: return\n"
        "        pendingLookupDomain = lookupDomain\n"
        ...
        "        val lookupDomain = pendingLookupDomain ?: return false\n"
        "        val origin = pendingOrigin ?: return false\n"
        "        if (!isAuthenticated) {\n"
        "            pendingLookupDomain = null\n"
        "            pendingOrigin = null\n"
        "            pendingPrivateAccess = false\n"
        "            return true\n"
        "        }\n"
        "        if (currentLoginOrigin() != origin) {\n"
        "            dismissCurrentLoginSelect()\n"
        "            return true\n"
        "        }\n"
        "        val privateAccess = pendingPrivateAccess\n"
        "        fetchUnlockedLogins(lookupDomain, origin, privateAccess) { logins ->\n"
        "            if (pendingLookupDomain != lookupDomain || pendingOrigin != origin) {\n"
        "                return@fetchUnlockedLogins\n"
        "            }\n"
        "            if (currentLoginOrigin() != origin) {\n"
        "                dismissCurrentLoginSelect()\n"
        "                return@fetchUnlockedLogins\n"
    ...
    path = target / (
        "mobile/android/android-components/components/feature/prompts/src/test/java/"
        "mozilla/components/feature/prompts/login/LoginPickerTest.kt"
    )
    s = read(path)
    anchor = "    private fun prepareSelectedSession(request: PromptRequest? = null): TabSessionState {\n"
    test = (
        "    @Test\n"
        "    fun `locked action confirms only after exact-origin authentication`() {\n"
        "        val gate = Login(\n"
        "            guid = \"fenix-privacy-login-gate:accounts.example\",\n"
        "            origin = \"https://accounts.example\",\n"
        "            username = \"Passwords\",\n"
        "            password = \"locked\",\n"
        "        )\n"
        "        val unlocked = login.copy(origin = gate.origin)\n"
        "        val gateRequest = PromptRequest.SelectLoginPrompt(\n"
        "            logins = listOf(gate),\n"
        "            generatedPassword = null,\n"
        "            onConfirm = { confirmedLogin = it },\n"
        "            onDismiss = { onDismissWasCalled = true },\n"
        "        )\n"
        "        val selectedSession = prepareSelectedSession(gateRequest)\n"
        "        var unlockRequested = false\n"
        "        var requestedPrivateAccess: Boolean? = null\n"
        "        loginPicker = LoginPicker(\n"
        "            store = store,\n"
        "            loginSelectBar = loginSelectBar,\n"
        "            sessionId = selectedSession.id,\n"
        "            isLockedLogin = { it.guid.startsWith(\"fenix-privacy-login-gate:\") },\n"
        "            lockedLoginLookupDomain = { \"accounts.example\" },\n"
        "            lockedLoginLabel = \"Unlock saved passwords\",\n"
        "            currentLoginOrigin = { gate.origin },\n"
        "            requestUnlock = { unlockRequested = true },\n"
        "            fetchUnlockedLogins = { lookupDomain, origin, privateAccess, onResult ->\n"
        "                assertEquals(\"accounts.example\", lookupDomain)\n"
        "                assertEquals(gate.origin, origin)\n"
        "                requestedPrivateAccess = privateAccess\n"
        "                onResult(listOf(unlocked))\n"
        "            },\n"
        "        )\n\n"
        "        loginPicker.handleSelectLoginRequest(gateRequest)\n"
        "        loginPicker.onOptionLongSelect(\n"
        "            gate.copy(origin = \"Unlock saved passwords\", username = \"\", password = \"\"),\n"
        "        )\n\n"
        "        assertTrue(unlockRequested)\n"
        "        assertEquals(null, confirmedLogin)\n"
        "        assertTrue(loginPicker.onBiometricResult(isAuthenticated = true))\n"
        "        assertEquals(true, requestedPrivateAccess)\n"
        "        assertEquals(unlocked, confirmedLogin)\n"
        "    }\n\n"
        "    @Test\n"
        "    fun `locked action fails closed when the current origin changed`() {\n"
        "        val gate = Login(\n"
        "            guid = \"fenix-privacy-login-gate:accounts.example\",\n"
        "            origin = \"https://accounts.example\",\n"
        "            username = \"Passwords\",\n"
        "            password = \"locked\",\n"
        "        )\n"
        "        val gateRequest = request.copy(logins = listOf(gate))\n"
        "        val selectedSession = prepareSelectedSession(gateRequest)\n"
        "        var unlockRequested = false\n"
        "        loginPicker = LoginPicker(\n"
        "            store = store,\n"
        ...
        "            isLockedLogin = { it.guid.startsWith(\"fenix-privacy-login-gate:\") },\n"
        "            lockedLoginLookupDomain = { \"accounts.example\" },\n"
        "            currentLoginOrigin = { \"https://different.example\" },\n"
        "            requestUnlock = { unlockRequested = true },\n"
        "        )\n\n"
        "        loginPicker.handleSelectLoginRequest(gateRequest)\n"
        "        loginPicker.onOptionSelect(gate)\n\n"
        "        assertTrue(!unlockRequested)\n"
        "        assertTrue(!loginPicker.onBiometricResult(isAuthenticated = true))\n"
        "    }\n\n"
    )
    s = replace_once(s, anchor, test + anchor, "Origin-bound login picker tests")
    write(path, s)
    ...
    s = replace_once(
        ...
        "                    lockedLoginLookupDomain = lockedLoginLookupDomain,\n"
        "                    lockedLoginLabel = lockedLoginLabel,\n"
        "                    currentLoginOrigin = currentLoginOrigin,\n"
    ...
    new = (
        ...
        "                    override val lockedLoginLookupDomain =\n"
        "                        PrivatePasswordAccess::lockedLookupDomain\n"
        "                    override val lockedLoginLabel\n"
        "                        get() = getString(R.string.private_password_unlock_label)\n"
        "                    override val currentLoginOrigin = {\n"
        "                        PrivatePasswordAccess.normalizeWebOrigin(\n"
        "                            getSafeCurrentTab()?.content?.url.orEmpty(),\n"
        "                        )\n"
        "                    }\n"
        ...
        "                        { lookupDomain: String, origin: String, privateAccess: Boolean,\n"
        "                            onResult: (List<Login>) -> Unit ->\n"
